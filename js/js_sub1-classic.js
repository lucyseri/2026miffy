...
import {sortedDataFn, classicData, pagerLengthFn, pagerLiCreateFn, pagerActiveFn, arrowBtnActiveFn} from './product-rawdata.js';
const productConUl = document.querySelector('section.product-sec .product-con ul');
let sortedData = classicData.sort(function(a, b){
...
select.addEventListener('change', function(e){
  ...
  if(select.value == 'latest'){
    sortedData = classicData.sort(function(a, b){
      return new Date(b.date) - new Date(a.date);
    });
    productConUl.innerHTML = sortedDataFn(sortedData, 0);
...
//pager li create
let pagerLiLength = pagerLengthFn(classicData);
