...
import {sortedDataFn, busanData, pagerLengthFn, pagerLiCreateFn, pagerActiveFn, arrowBtnActiveFn} from './product-rawdata.js';
const productConUl = document.querySelector('section.product-sec .product-con ul');
let sortedData = busanData.sort(function(a, b){
...
select.addEventListener('change', function(e){
  ...
  if(select.value == 'latest'){
    sortedData = busanData.sort(function(a, b){
      return new Date(b.date) - new Date(a.date);
    });
    productConUl.innerHTML = sortedDataFn(sortedData, 0);
...
//pager - length
let pagerLiLength = pagerLengthFn(busanData);
